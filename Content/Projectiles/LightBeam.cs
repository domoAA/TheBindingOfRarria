    ...
    public class LightBeam : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.timeLeft <= 360)
            {
                ...
                texture.DrawWithTransparency((Projectile.Center - Main.screenPosition - new Vector2(Main.screenWidth / 2, Main.screenHeight / 2)) * Main.GameZoomTarget + new Vector2(Main.screenWidth / 2, Main.screenHeight / 2), texture.Bounds, power * float.Sin(power), Color.LightYellow, 30, 3, Projectile.scale * (float.Sin(power) * float.Sqrt(Math.Abs(float.Sin(power))) + 1f), 0.02f, 4);
                texture.DrawWithTransparency((Projectile.Center - Main.screenPosition - new Vector2(Main.screenWidth / 2, Main.screenHeight / 2)) * Main.GameZoomTarget + new Vector2(Main.screenWidth / 2, Main.screenHeight / 2), texture.Bounds, power * float.Cos(power), Color.LightYellow, 30, 3, Projectile.scale * (float.Sin(power) * float.Sqrt(Math.Abs(float.Sin(power))) + 0.7f), 0.02f, 4);
