...
public class TripleFlail : ModProjectile
{
    ...
    public override void AI()
    {
            ...
            case AIState.Spinning:
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        if (!player.channel) // If the player releases then change to moving forward mode
                        {
                            ...
                            StateTimer = 0f;
                            Projectile.velocity = unitVectorTowardsMouse.RotatedBy(PiOver4 / 3f * (index - 1)) * launchSpeed + player.velocity;
                    ...
                    SpinningStateTimer += 0.5f;

                    Vector2 offsetFromPlayer = new Vector2(player.direction).RotatedBy((float)Math.PI * 10f * ((SpinningStateTimer + index * 20)/ 60f) * player.direction);

                    offsetFromPlayer = (offsetFromPlayer * new Vector2(0.6f, 1)).RotatedBy(unitVectorTowardsMouse.ToRotation());

                    Projectile.Center = mountedCenter + unitVectorTowardsMouse * 50 + offsetFromPlayer * 30f + new Vector2(0, player.gfxOffY);
        ...
        */
        ...
        if (Main.rand.NextBool(dustRate))
            Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Iron, 0f, 0f, 150, default(Color), 1.3f);
    }
    ...
    public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
    {
        ...
        modifiers.ScalingArmorPenetration += 0.33f;
    }
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        var chainTexture = TextureAssets.Chain41;
