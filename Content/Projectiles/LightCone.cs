...
using static TheBindingOfRarria.TheBindingOfRarria;
...
public class LightCone : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        var b = (Projectile.Center - Main.screenPosition + Projectile.Center.DirectionFrom(a).RotatedBy(PiOver2) * 113);
        var c = (Projectile.Center - Main.screenPosition + Projectile.Center.DirectionFrom(a).RotatedBy(-PiOver2) * 113);
        ...
        foreach (var target in Main.ActiveNPCs)
        {
            if (target.CanBeChasedBy() && !target.friendly)
            {
                ...
                {
                    target?.GetSlowed(State.Slow, 30);
                }
            }
        }
        foreach (var target in Main.ActiveProjectiles)
        {
            if (target.hostile)
            {
                if (IsPointInTriangle(target.Center - Main.screenPosition, a, b, c))
                {
                    target?.GetSlowed(State.Slow, 3000);
                }
            }
        }
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Projectile.scale = 3;
        texture.DrawWithTransparency(Projectile.Center - Main.screenPosition + Projectile.Center.DirectionTo(Main.player[Projectile.owner].Center) * Projectile.Center.Distance(Main.player[Projectile.owner].Center) / 2, Projectile.scale, Projectile.rotation, Color.LightYellow, 50);
        return false;
    }
