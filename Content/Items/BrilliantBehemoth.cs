...
public class BrilliantBehemothPlayer : ModPlayer
{
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        {
            if (!npc.friendly && npc.CanBeChasedBy() && !npc.immortal && (npc.Center == target.Center || npc.WithinRange(target.Center, ExplosionRadius)) && npc.whoAmI != target.whoAmI && target.aiStyle != NPCAIStyleID.DD2MysteriousPortal)
            {
